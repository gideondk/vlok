...
## Why no slowflake?
`Vlok` is dutch for "Flake" and while we have our cheese, wooden shoes and windmills. We love large numbers and going cheap the most! 
...
Because our architecture don't have a direct limit to a key size of 64 bits, `Vlok` uses 128 bits ids for more detailed timestamps and the possibility of cluster-wide uniqueness without introducing a managing architecture.
...
New ids can be generated by using the `generateID` function on the client, or by using the `generateIDs` in case you need multiple IDs at once. 
